...
const order: Screen[] = [
  ...
  "callRinging", "call", "help", "helpOpen", "setting", "settingDialog", "withdraw",
  "editProfile", "editContacts", "soundSetting", "permissionSetting",
];
    ...
    <div className="app-root">
      <div className="desktop-shell">
        <div className="phone-frame">
          ...
          {screen === "permissionToast" && <PermissionIntro go={go} sos={false} kakaoFailure />}
...
function Canvas({ children, className = "", style, onClick }: { children: React.ReactNode; className?: string; style?: React.CSSProperties; onClick?: () => void }) {
  return <main className={`canvas ${className}`} style={style} onClick={onClick}>{children}</main>;
}
...
function ImageScreen({ src, onClick }: { src: string; onClick: () => void }) {
  return <Canvas className="image-screen"><button className="image-button" onClick={onClick}><img src={src} alt="mock" /></button></Canvas>;
}

function Header({ title, back }: { title: string; back: () => void }) {
  return <header className="header"><button className="icon-button" onClick={back}><Icon name="keyboard_arrow_left" size={28} /></button><h1>{title}</h1></header>;
}
...
function Login({ go }: { go: Go }) {
  return (
    <Canvas className="login" style={useScale()}>
      <section className="login-brand"><img src={logo} alt="SafeCall" className="login-logo" /><p>내 손 안의 안심 통화 서비스</p><h1>SafeCall</h1></section>
      <section className="login-actions"><button className="kakao-button" onClick={() => go("kakao")}><Icon name="chat" size={18} />카카오 로그인<span /></button><button className="guest-button" onClick={() => go("home")}>로그인 없이 빠르게 사용하기</button></section>
    </Canvas>
  );
}

function Profile({ go, edit }: { go: Go; edit?: boolean }) {
  return (
    <Canvas className="profile" style={useScale()}>
      ...
      <div className={`profile-form ${edit ? "edit" : ""}`}>
        ...
        <p className="field-label">성별</p><div className="segment"><span>남자</span><span className="selected">여자</span></div>
      </div>
...
function Contacts({ go, modal, edit }: { go: Go; modal?: boolean; edit?: boolean }) {
  return (
    <Canvas className="contacts" style={useScale()}>
      ...
      <section className={`contact-area ${modal ? "under-modal" : ""} ${edit ? "edit" : ""}`}><ContactCard /><button className="plus" onClick={() => go("contactModal")}><Icon name="add" size={40} /></button></section>
...
function ContactModal({ go }: { go: Go }) {
  return <div className="contact-modal"><h2>연락처 추가</h2><ModalField label="이름" value="홍길동" /><ModalField label="관계" value="가족" /><ModalField label="전화번호" value="010-0000-0000" /><button onClick={() => go("contacts")}>추가</button></div>;
}
  ...
  return (
    <Canvas className="terms" style={useScale()}>
      ...
      <div className="terms-body">
        <p>개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다.</p>
        <p>개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다.</p>
        <p>개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다. 개인정보 처리 동의 내용입니다.</p>
        <p>개인정보 처리 동의 내용입니다.</p>
      </div>
      <label className="agree"><span />위 사항에 동의하십니까?</label><BottomButton label="다음" onClick={() => go("permissionBasic")} />
    </Canvas>
  );
}

function PermissionIntro({ go, sos, kakaoFailure }: { go: Go; sos: boolean; kakaoFailure?: boolean }) {
  return (
    <Canvas className="permission" style={useScale()}>
      <div className={kakaoFailure ? "dimmed-content" : ""}>
        <h1>{sos ? "SafeCall 이용을 위해 아래의 기능이 켜져 있는지 확인해주세요." : "SafeCall 이용을 위해 아래의 권한을 허용해주세요."}</h1>
        <section className="permission-list"><PermissionRow icon="mic" title="마이크" body="AI와 실시간 통화 이용 도중 음성 입력을 받기 위해 마이크 권한이 필요합니다." warning="마이크 권한을 허용하지 않을 시 서비스 이용이 불가합니다." />{!sos && <PermissionRow icon="location_on" title="위치" body="긴급 문자에 사용자의 현재 위치 안내 링크를 함께 제공하기 위해 위치 권한이 필요합니다." warning="위치 권한을 허용하지 않을 시 긴급 문자에 위치 안내 링크가 포함되지 않습니다." />}{sos && <PermissionRow icon="mic" title="긴급 SOS" body="앱 사용중이나 AI 안심 통화 서비스를 이용 중에 긴급한 상황 발생 시 안드로이드 시스템에 등록된 긴급번호로 전화를 연결하는 데 필요한 기능입니다." warning="긴급 SOS 기능은 SafeCall에서 제공하는 기능이 아닌, 안드로이드 시스템 자체에서 제공하는 기능입니다." />}</section>
        <p className="sos-warning">실제 119나 112에 신고가 갈 수 있으므로, SafeCall은 112 긴급 호출 기능을 제어할 수 없으므로 신중한 사용을 권장합니다.</p><BottomButton label="다음" onClick={() => go(sos ? "sosSystem" : "callPermission")} />
      </div>
...
function KakaoFailureModal({ onConfirm }: { onConfirm: () => void }) {
  return <div className="kakao-error-layer" aria-live="assertive"><div className="kakao-error-modal"><b>카카오 로그인에 실패하였습니다.</b><p>네트워크 연결을 확인하시고 다시 시도해주세요.</p><button type="button" onClick={onConfirm}>확인</button></div></div>;
}
...
function Complete({ go }: { go: Go }) {
  return <Canvas className="complete"><section><h1>설정이 모두 완료되었습니다.</h1><p>긴급 메시지 설정이 잘 완료되었는지 테스트해볼까요?</p></section><div className="two-buttons"><BottomButton dark label="건너뛰기" onClick={() => go("home")} /><BottomButton light label="확인" onClick={() => go("home")} /></div></Canvas>;
}
...
function CallSetupCheck({ go }: { go: Go }) {
  return <Canvas className="call-check"><Header title="모두 확인하셨나요?" back={() => go("personaPeople")} /><div className="check-icons"><div><Icon name="wifi_off" size={72} /><p>와이파이를 끄고 모바일<br />데이터를 사용해주세요</p></div><div><Icon name="volume_up" size={78} /><p>볼륨 4-5로 설정해주세요</p></div></div><p className="check-note">화면을 나가거나 긴급 SOS 통화를 사용할 경우 AI 통화는 자동으로 종료됩니다.</p><BottomButton label="다음" onClick={() => go("voiceLoading")} /></Canvas>;
}

function VoiceLoading({ go }: { go: Go }) {
  return <Canvas className="voice-loading" onClick={() => go("callRinging")}><h1>가상 통화를 준비하고 있습니다...</h1><div className="voice-bars"><span /><span /><span /><span /><span /></div><p>AI는 실제로 실행되지 않은 112 신고,<br />긴급 문자 발송 또는 위치 링크 전송이<br />완료되었다고 말하지 않습니다.<br /><br />AI는 사용자를 대신해 위험 여부를<br />판단하거나 긴급 문자 발송 또는<br />112 신고를 실행하지 않습니다.<br /><br />AI는 사용자에게 위험 인물과 대치,<br />추적 또는 촬영을 유도하지 않습니다.</p></Canvas>;
}
...
function HelpScreen({ go, open }: { go: Go; open?: boolean }) {
  const rows = ["SOS 기능을 다시 켜고 싶어요.", "긴급 연락처를 수정하고 싶어요.", "긴급 연락처를 입력하지 않아도 괜찮은가요?", "가상 전화를 소리 말고 진동이나 무음으로 받고 싶어요.", "음량 조절이 필수적인가요?"];
  return <Canvas className="help-screen"><Header title="도움말" back={() => go("setting")} /><section className="help-list">{rows.map((row, index) => <button key={row} onClick={() => index === 0 && go("helpOpen")}><span><b>Q.</b> {row}</span><Icon name="chevron_right" />{open && index === 0 && <p>SOS 기능은 기기 설정에서 다시 활성화할 수 있습니다.</p>}</button>)}</section></Canvas>;
}

function CallRinging({ go }: { go: Go }) {
  return <Canvas className="call-gradient"><section className="ring-title"><p>수신전화</p><h1>아빠</h1><img src={father} alt="아빠" /></section><p className="ring-warning"><Icon name="notifications" size={18} />안심통화는<br />실제 신고나 구조를<br />대신하지 않습니다.</p><div className="ring-actions"><button className="answer" onClick={() => go("call")}><Icon name="phone" size={32} /></button><button className="decline" onClick={() => go("home")}><Icon name="call_end" size={32} /></button></div></Canvas>;
}
...
function SoundSetting({ go, mode, setMode }: { go: Go; mode: string; setMode: (value: string) => void }) {
